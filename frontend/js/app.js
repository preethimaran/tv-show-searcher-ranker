...
const printFirstImage = async (queryString) => {
    try {
        ...
        try {
            ...
            const img_url = res.data.image.medium
...
const printFirstImagever1 = async (queryString) => {
    try {
        const config = { params: { q: queryString } }
        // const res = await axios.get(`https://api.tvmaze.com/search/shows?q=${query}`)
        ...
        if (res.data.length !== 0) {
            try {
                const img_url = res.data[0].show.image.medium
                const image = document.createElement('img')
                image.setAttribute('src', img_url)
                imgCntr.append(image)
            } catch (err) {
                console.log(err)
                const tvShow = res.data[0].show.name
